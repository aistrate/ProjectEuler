...
main = do contents <- readFile "sudoku.txt"
          ...
          let solvedPuzzles = map (applyOnSnd solvePuzzle) puzzles
          mapWithTitles printPuzzles solvedPuzzles
    where solvePuzzle = map gridToList . solveSudoku . makeSlotGrid
          applyOnSnd f (x, y) = (x, f y)
