...
squareDeltaNs minN = let ns = [minN-1..]
                         n2s = zipWith (*) (tail ns) ns
                         goodN2s = map (`div` 2) $ equalElems n2s (map (*2) n2s)
                         calcN n2 = (1 + intSqrt (1 + 4 * n2)) `div` 2
                     in map calcN goodN2s

squareDeltaNs2 minN = let ns = [minN-1..]
                          n2s = zipWith (*) (tail ns) ns
                          --goodN2s = map (`div` 2) $ equalElems n2s (map (*2) n2s)
                          --calcN n2 = (1 + intSqrt (1 + 4 * n2)) `div` 2
                      in n2s
