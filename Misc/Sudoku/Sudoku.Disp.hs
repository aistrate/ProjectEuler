...
main = do contents <- readFile "sudoku.txt"
          let puzzle = snd . (!! 36) . readPuzzles $ lines contents
          printPuzzle puzzle
          solvedPuzzle <- solvePuzzle puzzle
          putStrLn "\nSolutions:"
          printPuzzles solvedPuzzle
    where solvePuzzle = liftM (map gridToList) . solveSudoku . makeSlotGrid

readPuzzles :: [String] -> [(String, [[Digit]])]
readPuzzles = map readPuzzle . splitInto 10
    where readPuzzle ls = (head ls, map (map digitToInt) $ tail ls)

printPuzzle :: Show a => [[a]] -> IO ()
printPuzzle p = do putStrLn ""
                   mapM_ print p

printPuzzles = mapM_ printPuzzle

mapWithTitles :: (a -> IO ()) -> [(String, a)] -> IO ()
mapWithTitles prt = mapM_ printPair
    where printPair (title, puzzle) = do putStrLn $ "\n" ++ title
                                         prt puzzle
...
solveSudoku :: Grid Slot -> IO [Grid Slot]
solveSudoku g =
    do showPuzzle newGrid
       case minOptionCount newGrid of
           -1 -> return [newGrid]
           0  -> return []
           m  -> do let (i, Options ds) = firstOptionWithCount m newGrid
                    liftM concat $ mapM (solveFor i) ds
    where newGrid = calculateOptions g
          solveFor i d = solveSudoku $ replaceWithFixed newGrid i d

showPuzzle :: Grid Slot -> IO ()
showPuzzle p = do --printPuzzle $ gridToList p
                  printFormatted $ gridToList p
                  getChar
                  return ()
    where slotToDigit (Fixed d)     = " " ++ [intToDigit d] ++ " "
          slotToDigit (Options [d]) = "[" ++ [intToDigit d] ++ "]"
          slotToDigit (Options [])  = "(X)"
          slotToDigit _             = " . "
          printFormatted ds = do putStrLn ""
                                 mapM_ putStrLn $ map (concat . map slotToDigit) ds
