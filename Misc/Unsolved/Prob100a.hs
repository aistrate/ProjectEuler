-- (n / (n + m)) * ((n - 1) / (n + m - 1)) == 1/2

-- (n * (n - 1)) / ((n + m) * (n + m - 1)) == 1/2

-- 2 * n * (n - 1) == (n + m) * (n + m - 1)

-- 2n^2 - 2n == n^2 + nm - n + nm + m^2 - m

-- n^2 - n == m^2 + 2nm - m

-- m^2 + (2n - 1)m + (n - n^2) == 0

-- delta = b^2 - 4ac
-- m1 = (-b + sqrt(delta)) / 2a
-- m2 = (-b - sqrt(delta)) / 2a

-- a = 1
-- b = 2n - 1
-- c = n - n^2

-- delta = (2n - 1)^2 - 4(n - n^2)
--       = 4n^2 - 4n + 1 - 4n + 4n^2
--       = 8n^2 - 8n + 1

-- m = (sqrt(delta) + 1 - 2n) / 2
-- m = (sqrt(delta) + 1) / 2  - n

-- n + m == (sqrt(delta) + 1) / 2

-- [ minNPlusM = 10^12 + 1 ]

-- (sqrt(delta) + 1) / 2 >= minNPlusM
-- sqrt(delta) >= 2 * minNPlusM - 1
-- delta >= (2 * minNPlusM - 1)^2

-- 8n^2 - 8n + 1 >= (2 * minNPlusM - 1)^2
-- 8n^2 - 8n + 1 - (2 * minNPlusM - 1)^2 >= 0

-- a = 8; b = -8; c = 1 - (2 * minNPlusM - 1)^2
-- delta = 8^2 - 4 * 8 * (1 - (2 * minNPlusM - 1)^2)
--       = 32 ((2 * minNPlusM - 1)^2 + 1)

-- n >= (-8 + sqrt(delta)) / 16
-- n >= (4 * sqrt(2 * ((2 * minNPlusM - 1)^2 + 1)) + 8) / 16

-- n >= (sqrt(2 * ((2 * minNPlusM - 1)^2 + 1)) + 2) / 4

-- ex: minNPlusM = 20
--     ==> n >= (sqrt(2 * ((2 * 20 - 1)^2 + 1)) + 2) / 4
--           =  (sqrt(2 * (39^2 + 1)) + 2) / 4
--           =  (sqrt(3044) + 2) / 4
--           =~ (55.172 + 2) / 4
--           =  14.293
--         n >= 15


-- Algorithm:
-- Start with n = ceiling((sqrt(2 * ((2 * minNPlusM - 1)^2 + 1)) + 2) / 4)
-- Find minimum n for which delta = 8n^2 - 8n + 1 is a square number


{-minNPlusM :: Integer
--minNPlusM = 10^12
minNPlusM = 120

minN :: Integer
minN = ceiling ((sqrt (2 * ((2 * (fromIntegral minNPlusM) - 1)^2 + 1)) + 2) / 4)

squares :: [Integer]
squares = map (^2) [ceiling (sqrt (8*n^2 - 8*n + 1))..]
    where n = fromIntegral minN


firstEq (x:xs) (y:ys) | x == y    = x
                      | x <  y    = firstEq xs (y:ys)
                      | otherwise = firstEq (x:xs) ys
-}


import Data.Ratio
import Data.Maybe (fromJust)


calcMinN :: Integer -> Integer
calcMinN minNPlusM = ceiling ((sqrt (2 * ((2 * (fromIntegral minNPlusM) - 1)^2 + 1)) + 2) / 4)


equalElems :: [Integer] -> [Integer] -> [Integer]
equalElems (x:xs) (y:ys) | x == y    = x : equalElems xs ys
                         | x <  y    = equalElems xs (y:ys)
                         | otherwise = equalElems (x:xs) ys


squareDeltas :: Integer -> [(Integer, Integer)]
squareDeltas minN = let delta n = 8*n^2 - 8*n + 1
                        squares = map (^2) [(ceiling . sqrt . delta . fromIntegral $ minN)..]
                        deltaPair n = (delta n, n)
                        deltaPairs = map deltaPair [minN..]
                        sqDeltas = equalElems squares (map fst deltaPairs)
                    in map (deltaPair . fromJust . flip lookup deltaPairs) sqDeltas



squareDeltas2 :: Integer -> [Integer]
squareDeltas2 minN = let delta n = 8*n^2 - 8*n + 1
                         squares = map (^2) [(ceiling . sqrt . delta . fromIntegral $ minN)..]
                     in equalElems squares (map delta [minN..])


intSqrt :: Integral a => a -> a
intSqrt = truncate . sqrt . fromIntegral


nFromDelta :: Integer -> Integer
nFromDelta delta = let d = 64 + 32 * (delta - 1)
                   in (8 + intSqrt d) `div` 16


nmPairs :: Integer -> [(Integer, Integer)]
{-nmPairs minN = let nmPair (delta, n) = (n, (intSqrt delta + 1) `div` 2 - n)
               in map nmPair . squareDeltas $ minN-}
nmPairs minN = let nmPair delta = let n = nFromDelta delta 
                                      m = (intSqrt delta + 1) `div` 2 - n
                                  in (n, m)
               in map nmPair . squareDeltas2 $ minN


probability :: (Integer, Integer) -> Rational
probability (n, m) = let nn = n % 1
                         mm = m % 1
                     in nn / (nn + mm) * (nn - 1) / (nn + mm - 1)


main = do let p@(n, m) = head . nmPairs . calcMinN $ 10^6 + 1
          print p
          print $ n + m
          print $ probability p


-- For n + m >= 10^6 + 1:
-- (3312555,1372105)
-- 4684660
-- 1 % 2
