...
-- squareDeltas minN = filter (\d -> d == (intSqrt d)^2) $ map calcDelta [minN..]
squareDeltas minN = let minDeltaSqrt = 2 * ((ceiling . sqrt . fromIntegral . calcDelta $ minN) `div` 2 - 1) + 1
                        squares = map (^2) [minDeltaSqrt,minDeltaSqrt+2..]
                    in equalElems squares (map calcDelta [minN..])
